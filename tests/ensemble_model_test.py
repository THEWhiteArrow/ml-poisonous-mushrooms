...
def ensemble_model() -> EnsembleModel:
    ...
    ensemble_model_path = (
        Path(f"{PathManager.OUTPUT_DIR_PATH.value}")
        / f"{PrefixManager.ENSEMBLE_PREFIX.value}{config['model_run']}"
        / f"ensemble_model_{config['model_run']}.pkl"
    )
...
def test_ensemble_model_prediction():
    ...
    ensemble_model = cast(EnsembleModel, pickle.load(open(ensemble_model_path, "rb")))
...
def test_ensemble_models_params(
    ensemble_model: EnsembleModel, ensemble_config: EnsembleConfigDict
):
    """
    ...
    for i, model in enumerate(ensemble_model.models):
        ...
        result_params = json.loads(
            results.loc[results["name"].eq(combination_name), "params"]
            .iloc[0]
            .replace("'", '"')
        )
...
def test_ridge_models_have_same_predictions(ensemble_model: EnsembleModel):
    ...
    e_ridge_prediction = [
        prediction
        for prediction in e1.predictions
        if prediction.name is not None and "ridge" in str(prediction.name).lower()
    ][0]
    ...
    ridge = [
        model for model in e2.models if "ridge" in model.__class__.__name__.lower()
    ][0]

    pipeline = ProcessingPipelineWrapper().create_pipeline(model=ridge)
    features_in = [
        features
        for (i, features) in enumerate(ensemble_model.combination_feature_lists)
        if "ridge" in ensemble_model.combination_names[i].lower()
    ][0]

    pipeline = pipeline.fit(X_train[features_in], y_train)
    ridge_prediction = pd.Series(
        pipeline.predict(X_test[features_in]), index=X_test.index, name="ridge"
    )
